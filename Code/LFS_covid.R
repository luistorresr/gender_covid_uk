...
LFS21_JN <- LFS21_JN %>% mutate(QUARTER = 11)
...
LFS_variables <- c(
        ...
        "CASENO", # person unique identifier
        ...
        "EVHM98", # Do you ever do any paid or unpaid work at home for your (main) job? (filter by those who do not work at home: HOME = 2, 3 or 4)
...
LFS_all <- rbind(t4, t5, t6, t7, t8, t9, t10) # working database
q_LFS_all <- as.data.frame(label(LFS_all)) # variable labels
l_LFS_all <- get_labels(LFS_all, values = "n") # value labels 
...
rm(LFS_variables) # delete the variable selection file
rm(l_LFS_all) # delete the combined dataset labels
rm(LFS_all) # delete the combined dataset 
...
LFS_clean$QUARTER <- set_labels(LFS_clean$QUARTER, labels = c("Apr-Jun 20" = 4, "May-Jul 20" = 5, "Jun-Aug 20" = 6,
                                                                              "Jul-Sep 20" = 7, "Aug-Oct 20" = 8, "Sep-Nov 20" = 9,
                                                                              "Oct-Dec 20" = 10))
...
LFS_clean$RESTMR6_2 <- set_labels(LFS_clean$RESTMR6_2, labels = c("Permanent job" = 1, "Seasonal job" = 2, "Fixed contract" = 3,
...
LFS_full <- LFS_clean # no changes

LFS_log <- LFS_full[!duplicated(CASENO), ]

LFS_cros <- LFS_full %>% distinct(CASENO, .keep_all = TRUE)
...
## Sample size by quarter and sex
...
n_tot_sex <- LFS_full %>%
        filter(AGE >= 18 & AGE <= 64 & IOUTCOME <= 2) %>%
        group_by(as_label(QUARTER), as_label(SEX)) %>%
        summarise(n = n()) %>% 
        pivot_wider(names_from = `as_label(SEX)`, values_from = n)
